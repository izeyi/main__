(function() {
  ...
  canvas.on("mouse:up", function(options) {
    var xy = transformMouse(options.e.offsetX, options.e.offsetY);
    mouseTo.x = xy.x;
    mouseTo.y = xy.y;// drawing();
    drawingObject = null;
    moveCount = 1;
    doDrawing = false;
  });
  canvas.on("mouse:move", function(options) {
    if (moveCount % 2 && !doDrawing) return;//减少绘制频率
    moveCount++;
    var xy = transformMouse(options.e.offsetX, options.e.offsetY);
    mouseTo.x = xy.x;
    mouseTo.y = xy.y;
  ...
  jQuery("#toolsul").find("li").on("click", function() {
    //设置样式
    jQuery("#toolsul").find("li>i").each(function() {jQuery(this).attr("class", jQuery(this).attr("data-default"))});//修改背景图
    jQuery(this).addClass("active").siblings().removeClass("active");//修改背景色
  ...
  jQuery("#swatches").find("span").each(function() {jQuery(this).css("background", jQuery(this).attr("data-color"))});//添加背景颜色
  jQuery("#swatches").find("span").on("click",function(){color = jQuery(this).attr("data-color");});//修改颜色
  //绘画方法
  function drawing() {
    ...
    if (canvasObject) { // canvasObject.index = getCanvasObjectIndex();
